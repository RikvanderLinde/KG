...
class TED_TEDSystemDev_Overview {
  public static function showOverview(){
    ...
    if (!isset($_REQUEST['user_id'])) {
      ...
      $s.= '<div class="filter"><form action="/pages/tedsystemdev/" method="GET">';
      ...
      foreach($teds as $ted) {
        $s.= '<tr>';
        $s.= '<td><a href="/pages/tedsystemdev/?user_id='.$ted['user_id'].'">'.strip_html($ted['username']).'</a></td>';
        ...
        if ($user[0]['display_style_group_id'] !== 19 && !$modtools) {
          ...
          if (isset($_REQUEST['vote']) && ($_REQUEST['vote']=='up'||$_REQUEST['vote']=='down')) {
            ...
            if (!in_array($vote.'~'.$visitor->user_id, $voters)) { // Already voted this answer?
              ...
              header("Location: /pages/tedsystemdev/?user_id=".$user[0]['user_id']); /* Redirect browser */
              exit();
            }
          ...
          if ($user[0]['display_style_group_id'] == 19) {
            $up = '<div class="votebutton"><a href="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'&vote=up" class="plus">+</a></div>';
            $down = '<div class="votebutton"><a href="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'&vote=down" class="minus">-</a></div>';
          } else {
            $up = '';
            $down = '';
          }
          ...
          $s.= '<a class="button" href="/pages/tedsystemdev/">Go back to overview</a>';
          if ($modtools && ($ted[0]['status'] == 0 || $ted[0]['status'] == 4 || $ted[0]['status'] == 1) && $user[0]['display_style_group_id'] == 19) {
            $s.= '<a class="button extend modbutton" href="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'&modaction=extend">EXTEND</a>';
            $s.= '<a class="button fail modbutton" href="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'&modaction=fail">FAIL</a>';
            $s.= '<a class="button pass modbutton" href="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'&modaction=pass">PASS</a>';
          } elseif ($modtools && ($ted[0]['status'] == 3)) {
            $s.= '<a class="button pass modbutton" href="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'&modaction=restart">RESTART TRIAL</a>';
          }
          ...
          $s.= '<td>:</td>';
          if ($modtools) {
            $game_form = '<form action="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'" method="POST">
          ...
          if ($modtools) {
            ...
            $date_form = '<form action="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'" method="POST">
                            <input type="hidden" name="_xfToken" value="'.$visitor->csrf_token_page.'" />
          ...
          if ($modtools) {
            ...
            $s.= '<td>:</td>';
            $ts_uid = $ted[0]['ts_uid'];
            $ts_uid_form = '<form action="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'" method="POST">
                            <input type="hidden" name="_xfToken" value="'.$visitor->csrf_token_page.'" />
          ...
          $s.= '<tr>';
          $s.= '<td colspan="2">';
          $s.= '<form action="/pages/tedsystemdev/?user_id='.$user[0]['user_id'].'" method="POST">
                  <input type="hidden" name="_xfToken" value="'.$visitor->csrf_token_page.'" />
